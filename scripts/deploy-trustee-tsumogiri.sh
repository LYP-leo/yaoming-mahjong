...
backup="$project/backups/trustee-tsumogiri-20260909"
archive=/tmp/mahjong-trustee-tsumogiri-20260909.tar.gz
jar=/tmp/mahjong-trustee-tsumogiri-20260909.jar
...
test "$(sha256sum "$project/backend/target/mahjong-server-0.1.0.jar" | cut -d' ' -f1)" = 07d0819b5b54ffd61acbbe5c68080e827715f5031c2dad4a4c2d6a199f36ab9b
test "$(sha256sum "$site/index.html" | cut -d' ' -f1)" = a7776cc0b379bea314288dfb37d8b55072179c91d77cfce7fcd49427416db13f
check_rooms() {
...
cp "$backup/stage/frontend/dist/index.html" "$site/index.trustee-tsumogiri.next"
chmod 644 "$site/index.trustee-tsumogiri.next"
mv -f "$site/index.trustee-tsumogiri.next" "$site/index.html"
...
printf 'Pure trustee and compact latest-discard released. Recovery: %s\n' "$backup"
