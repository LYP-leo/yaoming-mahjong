...
backup="$project/backups/contextual-win-hint-20260912"
archive=/tmp/mahjong-contextual-win-hint-20260912.tar.gz
...
test "$(sha256sum "$site/index.html" | cut -d' ' -f1)" = 2eaf641008af91132bc141dc1f6587d108af2176a8432ca3b9cdb91b5263563b
...
allowed = {
    'frontend/src/yaoming/TableView.vue',
    'frontend/src/yaoming/four-player-rules.test.ts',
    'frontend/src/yaoming/contextual-win-hint.test.ts',
    'frontend/src/yaoming/stable-layout.test.ts',
    'frontend/src/yaoming/dev/StableTablePreview.vue',
    'docs/CONTEXTUAL_WIN_HINT_PLAN.md',
    'docs/CONTEXTUAL_WIN_HINT_TEST_REPORT.md',
    'scripts/deploy-contextual-win-hint.sh',
}
seen = set()
...
tar -czf "$backup/source-before.tar.gz" -C "$project" frontend/src/yaoming/TableView.vue frontend/src/yaoming/four-player-rules.test.ts frontend/src/yaoming/stable-layout.test.ts frontend/src/yaoming/dev/StableTablePreview.vue frontend/dist
tar -czf "$backup/public-before.tar.gz" -C "$site" .
cp "$site/index.html" "$backup/index-before.html"
rollback() {
  code=$?
  trap - ERR
  cp "$backup/index-before.html" "$site/index.contextual-hint-rollback.next"
  chmod 644 "$site/index.contextual-hint-rollback.next"
  mv -f "$site/index.contextual-hint-rollback.next" "$site/index.html"
...
cp "$backup/stage/frontend/dist/index.html" "$site/index.contextual-hint.next"
chmod 644 "$site/index.contextual-hint.next"
mv -f "$site/index.contextual-hint.next" "$site/index.html"
