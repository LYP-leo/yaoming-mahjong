...
backup="$project/backups/four-player-20260912"
archive=/tmp/mahjong-four-player-20260912.tar.gz
jar=/tmp/mahjong-four-player-20260912.jar
...
test "$(sha256sum "$project/backend/target/mahjong-server-0.1.0.jar" | cut -d' ' -f1)" = 0f9761d02b5bd45e6f83eeae5afa9a22f6127abc659bf2addfccb881bfe354d4
test "$(sha256sum "$site/index.html" | cut -d' ' -f1)" = 57fe1dcfa3b620b31088b43f56d65223d576cc994d5d25dc750e04c0c21f192a
python3 - "$archive" <<'PY'
import sys, tarfile
...
for(const room of lobby){assert.equal(room.ruleId,'yaoming-3p');assert.equal(room.capacity,3);}
...
trap - ERR
printf 'Published four-player rules. Backup: %s\n' "$backup"
