...
$runtimeArtifact = 'D:\mahjong\artifacts\trustee-tsumogiri-20260909'
$runtimeTarget = 'D:\mahjong\backend\target'
$runtimeBefore = 'D:\mahjong\artifacts\trustee-tsumogiri-20260909\local-target-before'
$runtimeArchive = 'D:\mahjong\artifacts\trustee-tsumogiri-20260909\local-runtime-before.tar.gz'
$runtimeDataArchive = 'D:\mahjong\artifacts\trustee-tsumogiri-20260909\local-data-stopped.tar.gz'
$runtimeSource = 'D:\mahjong\artifacts\trustee-tsumogiri-20260909\backend\target\mahjong-server-0.1.0.jar'
...
function Assert-OldRuntime {
    $process = Get-CimInstance Win32_Process -Filter 'ProcessId = 35800'
    if (-not $process -or $process.ExecutablePath -ne $runtimeOldJava -or $process.CommandLine -ne $runtimeOldCommand) { throw 'PID 35800 no longer matches the verified old runtime.' }
    $owners = @(Get-NetTCPConnection -LocalPort 8080 -State Listen | Select-Object -ExpandProperty OwningProcess -Unique)
    if ($owners.Count -ne 1 -or $owners[0] -ne 35800) { throw 'Local port 8080 no longer belongs exclusively to PID 35800.' }
...
try {
    ...
    Assert-OldRuntime
    $oldProcess = Get-Process -Id 35800
    Stop-Process -Id 35800
    $runtimeStopped = $true
