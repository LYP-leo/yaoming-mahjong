...
$runtimeBefore = 'D:\mahjong\artifacts\no-furiten-20260909\local-target-before'
$runtimeArchive = 'D:\mahjong\artifacts\no-furiten-20260909\local-runtime-before.tar.gz'
$runtimeDataArchive = 'D:\mahjong\artifacts\no-furiten-20260909\local-data-stopped.tar.gz'
$runtimeSource = 'D:\mahjong\artifacts\no-furiten-20260909\backend\target\mahjong-server-0.1.0.jar'
...
function Assert-OldRuntime {
    $process = Get-CimInstance Win32_Process -Filter 'ProcessId = 32212'
    if (-not $process -or $process.ExecutablePath -ne $runtimeOldJava -or $process.CommandLine -ne $runtimeOldCommand) { throw 'PID 32212 no longer matches the verified old runtime.' }
    $owners = @(Get-NetTCPConnection -LocalPort 8080 -State Listen | Select-Object -ExpandProperty OwningProcess -Unique)
    if ($owners.Count -ne 1 -or $owners[0] -ne 32212) { throw 'Local port 8080 no longer belongs exclusively to PID 32212.' }
...
try {
    ...
    Assert-OldRuntime
    $oldProcess = Get-Process -Id 32212
    Stop-Process -Id 32212
    $runtimeStopped = $true
