...
backup="$project/backups/room-lifecycle-20260912"
archive=/tmp/mahjong-room-lifecycle-20260912.tar.gz
jar=/tmp/mahjong-room-lifecycle-20260912.jar
...
test "$(sha256sum "$project/backend/target/mahjong-server-0.1.0.jar" | cut -d' ' -f1)" = 0a35f3b5ad64ec929fec08f421dfaf03e5ffb6dd4794bb3e49c01777acf43bac
test "$(sha256sum "$site/index.html" | cut -d' ' -f1)" = 885002b07c46610823e660b3b667395e797c092556c08d8c6f7cbca5869fdbaa
python3 - "$archive" <<'PY'
import sys, tarfile
...
cp "$backup/stage/frontend/dist/index.html" "$site/index.room-lifecycle.next"
chmod 644 "$site/index.room-lifecycle.next"
mv -f "$site/index.room-lifecycle.next" "$site/index.html"
systemctl start nginx
...
trap - ERR
printf 'Published waiting-room exit and empty-room expiry. Backup: %s\n' "$backup"
