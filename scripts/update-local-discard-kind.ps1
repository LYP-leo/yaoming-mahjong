...
$runtimeArtifact = 'D:\mahjong\artifacts\discard-kind-20260909'
$runtimeTarget = 'D:\mahjong\backend\target'
$runtimeBefore = 'D:\mahjong\artifacts\discard-kind-20260909\local-target-before'
$runtimeArchive = 'D:\mahjong\artifacts\discard-kind-20260909\local-runtime-before.tar.gz'
$runtimeDataArchive = 'D:\mahjong\artifacts\discard-kind-20260909\local-data-stopped.tar.gz'
$runtimeSource = 'D:\mahjong\artifacts\discard-kind-20260909\backend\target\mahjong-server-0.1.0.jar'
...
function Assert-OldRuntime {
    $process = Get-CimInstance Win32_Process -Filter 'ProcessId = 25876'
    if (-not $process -or $process.ExecutablePath -ne $runtimeOldJava -or $process.CommandLine -ne $runtimeOldCommand) { throw 'PID 25876 no longer matches the verified old runtime.' }
    $owners = @(Get-NetTCPConnection -LocalPort 8080 -State Listen | Select-Object -ExpandProperty OwningProcess -Unique)
    if ($owners.Count -ne 1 -or $owners[0] -ne 25876) { throw 'Local port 8080 no longer belongs exclusively to PID 25876.' }
...
try {
    ...
    Assert-OldRuntime
    $oldProcess = Get-Process -Id 25876
    Stop-Process -Id 25876
    $runtimeStopped = $true
