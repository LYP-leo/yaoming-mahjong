...
backup="$project/backups/no-furiten-20260909"
archive=/tmp/mahjong-no-furiten-20260909.tar.gz
jar=/tmp/mahjong-no-furiten-20260909.jar
...
PY
test "$(sha256sum "$project/backend/target/mahjong-server-0.1.0.jar" | cut -d' ' -f1)" = 9c8537ab2bca44ec68ce29243ffb57c697963eb3363df045d0e9cfd70becdaab
test "$(sha256sum "$site/index.html" | cut -d' ' -f1)" = 49e7744c445e4203b80c7e28e86d660cd7fc140b4eceec31642720b74aeec290
check_rooms() {
...
cp "$backup/stage/frontend/dist/index.html" "$site/index.no-furiten.next"
chmod 644 "$site/index.no-furiten.next"
mv -f "$site/index.no-furiten.next" "$site/index.html"
...
node --input-type=module -e "const r=await fetch('http://127.0.0.1:8080/api/yaoming/rules');if(!r.ok)throw Error('rules check failed');const v=await r.json();if(!v.notes.some(n=>n.includes('打过或放过同种牌仍可点和')))throw Error('new rule missing');if(v.notes.some(n=>n.includes('下一次摸牌前不能点和')))throw Error('old restriction remains');console.log('New no-furiten rule verified');"
...
printf 'No-furiten correction released. Recovery: %s\n' "$backup"
